  ...
  input %>%
    dplyr::mutate(
      mb.genres = purrr::pmap(
        list(track.mb.genres, album.mb.genres, artist.mb.genres),
        choose_tag_set_from_tracks_albums_artists,
        .progress = 'Combining tag sets in the data ...'
      )
    )
}

choose_tag_set_from_tracks_albums_artists <- function(
  track_tags,
  album_tags,
  artist_tags
) {
...
filter_non_empty_tags <- function(input) {
  ...
  input$is.tag_name <- lapply(input$mb.genres, function(x) {
    ...
    as.logical()
  dplyr::filter(input, is.nonempty & is.tag_name) %>%
    dplyr::select(-is.nonempty, -is.tag_name)
}

filter_music_tags <- function(input, non_music_tags) {
  input$mb.genres <- erase_non_music_tags(input$mb.genres, non_music_tags) # takes a while
  input$is.nonempty <- lapply(input$mb.genres, function(x) nrow(x) > 0) %>%
    as.logical()
  dplyr::filter(input, is.nonempty) %>%
...
erase_non_music_tags <- function(mb_tags, non_music_tags) {
  mb_tags_valid <- purrr::map(
    mb_tags,
    function(frame) {
      frame %>% dplyr::filter(!tag_name %in% non_music_tags)
    },
...
unpack_mb_genre_tags <- function(input) {
  ...
  unpacked_mb_tags <- unpack_genre_tags(mb_tags_valid$mb.genres)
  input_select |>
