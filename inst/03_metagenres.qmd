...
<!-- Eigentlich müsste es sogar so sein, das,,s der bei unterschreiten der Pfad mit der kleineren Summe eingeklappt wird, oder? -->
...
flowchart TD
    ...
    E -->|No| G["Merge genre with parent and all siblings and all grandchildren of parent"]
...
:::{.plot-container}
```{r net_dc}
plot_network_graph(graph = dc_suggested_solution$metagenre_graph, 
                   mapping = dc_suggested_solution$mapping, 
                   sizemode = "metagenre", 
                   height = 1000, 
                   interactive = TRUE)
```	
