...
```{r gini_plot_musicbrainz, fig.width=8, fig.height=6}
plot <- plot_tuning_results_tree(
  mb_tuning$ginis, 
  xlimits = c(0, min(c(max(mb_tuning$ginis$n_metagenre), 50)))
)
ggplot2::ggsave(
  filename = "../reports/paper_v1/figures/folding_gini_trajectory.png",
  plot = plot,
  width = 180,
  height = 115,
  dpi = 300,
...
```
Within-hierarchy-level uniformity of distribution (weighted Gini coefficient, [0-1]) for different minimum song per genre counts when folding the MusicBrainz genre tree.   .
:::
...
:::{#fig-spotify-gini}
```{r gini_plot_spotify}
plot_tuning_results_tree(
  s_tuning$ginis, 
  xlimits = c(0, min(c(max(s_tuning$ginis$n_metagenre), 50)))
)
```
