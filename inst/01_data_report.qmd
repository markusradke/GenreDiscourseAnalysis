...
We ground our analysis on a dataset with `r format(n_poptrag, big.mark = ",")` tracks that were popular in Germany within the last 50 years (**POPTRAG**, **Pop**ular **Tra**cks in **G**ermany).  The tracks were collected from the following sources:
...
```{r musicbrainz_most_common_tags}
most_common_tags <- mb_long |> 
  dplyr::count(tag_name, sort = TRUE) |>
  dplyr::slice_head(n = 20) |>
  dplyr::mutate(perc = sprintf("%.0f%%",  n / n_mb * 100)) 
# put Other level at the end of the factor
most_common_tags$tag_name <- forcats::fct_reorder(most_common_tags$tag_name, most_common_tags$n, .desc = FALSE)
most_common_tags$tag_name <- forcats::fct_relevel(most_common_tags$tag_name, "Other", after = 0)


ggplot2::ggplot(most_common_tags, ggplot2::aes(y = tag_name, x = n)) +
  ggplot2::geom_col(fill = 'grey65') +
  ggplot2::geom_text(ggplot2::aes(label = ifelse(!is.na(perc), perc, "")), hjust = -0.2) +
  ggplot2::labs(y = "", x = "Number of Track Assignments") +
  ggplot2::geom_vline(xintercept = n_mb, linetype = "dashed", color = "#3e578e") +
  ggplot2::annotate("text", x = n_mb, y = 1, label = sprintf("N = %s", format(n_mb, big.mark=",")), color = "#3e578e", hjust = 1.1) +
  ...
  ggplot2::theme(
    ...
    axis.text.x = ggplot2::element_text(color="grey45", size=10),
    axis.title.x = ggplot2::element_text(color="grey45")
  )
  ggplot2::ggsave("../reports/paper_v1_poetics/figures/fig01_most_common_tags.png", width = 6, height = 4)
```
