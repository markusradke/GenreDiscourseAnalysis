...
test_that("get_graph_root errors when there is not exactly one root", {
  # A->B and C->D => two sinks
  g_multi <- igraph::make_graph(c("A", "B", "C", "D"), directed = TRUE)
  expect_error(get_graph_root(g_multi))
  # cycle => no sink
  g_none <- igraph::make_graph(c("A", "B", "B", "C", "C", "A"), directed = TRUE)
  expect_error(get_graph_root(g_none))
})

test_that("get_subgraph returns the correct component incl node", {
  g <- igraph::make_graph(c("A", "B", "B", "C"), directed = TRUE)
  sg <- get_subgraph(g, "B")
