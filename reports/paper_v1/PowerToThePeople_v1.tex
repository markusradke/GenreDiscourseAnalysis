...
recently proposed a feature space spanning four dimensions—musical style, release context, lyrical content, and popularity to operationalize the multi-dimensional constitution of genre following Fabbri's~(\citeyear{fabbriTheoryMusicalGenres1982}) approach.
...
\subsection{Results: Community Music Genre Tree for Popular Music in Gemrmany (COMGET-G)}
...
\section{RQ3: External Validity and ML-Classifier}
\subsection{Methods: Single Labels, Content Validity, Features, Workflow}
To compare the COMGET-G genre assignments to other assginments frequently used in MIR and computational musicology and for training a classifier, each track in the corpus obtained a \textit{single COMGET-G genre label} via majority voting: all valid original \textit{MusicBrainz} tags assigned to a track were mapped to their \textit{category assignment} in folded COMGET-G, then each track received the \textit{category} tag with the highest summed votes. Ties were resolved by selecting the \textit{genre category} with lower total assignment probability in the full dataset, thereby favoring more detailed \textit{genre category} assignments. We derived \textit{single genre labels} for each track at four levels of granularity (COMGET-G5, COMGET-G12, COMGET-G25, COMGET-G32). For demonstrating the content validity of our solutions, we compared resulting \textit{genre category assignments} with comparable granularity to genres labels assinged by the predicted \textit{Rosamerica}-Genres from \textit{AcousticBrainz}~\citep{jimenez2020tensorflow}, community assignments from the \textit{Discogs}~(\citeyear{Discogs2024}) folksonomy, and genres retrieved from the API of the music streaming platform \textit{Deezer}~(\citeyear{Deezer_nodate}). We analyzed similarity using contingency tables.
...
\begin{figure*}[htbp]
  ...
  \begin{subfigure}[b]{\columnwidth}
    ...
    \caption{COMGET-G5 and 8 \textit{Essentia}-Predicted \textit{Rosamerica} Genres}
    \label{fig:rq3_rosa}
  \end{subfigure}
  \hfill
  \vspace{1.5 cm}
