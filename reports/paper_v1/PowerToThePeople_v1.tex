...
FOLDCAT addresses diverse practical needs while enabling cross-study comparison. The folding algorithm generates optimally balanced genre categories at multiple granularities using a single statistical criterion (Gini coefficient) across hierarchy levels. In addition, the underlying genre category assignments stay probabilistic. Hence, the framework quantifies achievable performance ceilings through community certainty, since models cannot exceed human consistency~\citep{sturmReviewValidityIts2023}. Strong correlations between certainty and classifier performance validate this approach, grounding MIR in listener perception limits. Classifier performance also demonstrates general learnability of the identified genre categories, while systematic confusion patterns reveal fuzzy boundaries (rock subgenres), stylistic permeability (electronic across pop/jazz/indie), and hierarchical nuance (heavy metal versus metal asymmetry). These patterns inform which granularities suit different research questions and reaffirm that genre labels remain learnable from the dimensions suggested by Lepa and Radke (2026). 

%TODO: INtegrate into paragraph before
% Artist-level features dominate across granularities: distribution genre flags (Spotify artist tags) emerge as strongest predictors, with artist demographics outweighing audio features. Release milieu guides perception~\citep{nealeGenre1980}. Stylistic features contribute secondarily—audio distinguishes broad categories, but lyric features remain weak, suggesting need for LLM embeddings~\citep{prevedelloLyricsSuccessEmbedding2024}. Popularity emerges at finer resolutions, where commercial success itself becomes a genre signal~\citep{holtGenrePopularMusic2007}, particularly for mainstream pop detection. 
...
%TODO Integrate into boundary discussions with one sentence
% Genre-specific confusion patterns reveal discourse boundaries: \textit{rock} subgenre overlap (\textit{alternative}, \textit{pop rock}, \textit{indie}) indicates fuzzy boundaries; \textit{electronic} permeability across \textit{pop}, \textit{jazz}, and \textit{indie} suggests stylistic fluidity; \textit{schlager} and \textit{hip hop} separation demonstrates distinct release-milieus


The method enables diverse applications for cultural research: % LIST A FEW EXAMPLES WITH STUDIES
Stakeholder applications extend beyond technical classification: the method enables radio programmers and playlist editors to understand how audiences actually organize music; A\&R teams to identify gaps between industry positioning and listener reception; marketing departments to recognize which subcultural boundaries audiences perceive versus industry-imposed categories; music journalists to ground critical language in empirical listener consensus; platform designers to align recommendation logic with listener mental models. 


\subsection{Limitations and Future Directions}
%TODO improve these paragraphs
Several constraints limit generalization of our method's results to other contexts. \textit{MusicBrainz} launched in 2000 without individual vote timestamps, so COMGET-G aggregates roughly 25 years of listener votes across a period when genre boundaries and perception likely shifted. Platform user demographics and expertise remain opaque (approximately 1.2 million anonymous editor accounts), preventing assessment of demographic or expertise bias. Western and English-language bias affects the platform. Popularity bias excluded 36\% of tracks with insufficient votes—acceptable for popular music focus but problematic for niche genres, potentially biasing against non-Western genres. The method remains portable to any folksonomy, but resulting taxonomies remain culturally and linguistically specific: language-specific terms (e.g., \textit{Schlager} in German context) may not transfer to other linguistic communities, limiting cross-cultural comparability of derived categories.


Methodologically, collinearity among predictors complicates feature importance interpretation and may cause instability across cross-validation folds. Single-label reduction loses multi-tag nuance for classification, though this simplification proves necessary for many applied research tasks. Lyric features contributed minimally to classifier performance, suggesting current bag-of-words representations fail to capture genre-relevant content; LLM-based embeddings may improve lyric signal~\citep{prevedelloLyricsSuccessEmbedding2024}.
