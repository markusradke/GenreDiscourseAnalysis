...
one_track_tags_mb <- dplyr::filter(dplyr::count(mb_long, tag_name), n == 1) |> nrow()
```

We filtered out all songs without a genre tag or with only non-music genre tags on MusicBrainz (`r format(n_diff_mb, big.mark = ",")` tracks, `r round(n_diff_mb / n_poptrag * 100)`%). Non-music genre tags were: `r paste(sprintf('"%s"', mb_non_music_tags), collapse = ', ')`.
...
one_track_tags_dc <- dplyr::filter(dplyr::count(dc_long, tag_name), n == 1) |> nrow()
```
...
```{r spotify_genres}
poptrag_filtered_s_genres <- readRDS('../data/filtered_s.rds')
s_long <- readRDS('../data/filtered_s_long.rds')
s_non_music_tags <- readRDS('../data/s_non_music_tags.rds')

n_diff_s <- n_poptrag - nrow(poptrag_filtered_s_genres)
...
one_track_tags_s <- dplyr::filter(dplyr::count(s_long, tag_name), n == 1) |> nrow()
```
...
This amounts to **N~Spotifys~ = `r format(nrow(poptrag_filtered_s_genres), big.mark = ",")` tracks** with Spotify music genre tags.
...
:::{.callout-note}
Also for the Spotify dataset, all votes for tags per track were set to 1. Also Spotify does not provide a number of votes per tag.
:::
:::{.callout-important}
