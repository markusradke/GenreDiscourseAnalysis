...
```{r musibrainz_genres}
get_unique_tags_from_mb_taglist <- function(mb_tags) {
...
tags_per_track <- dplyr::count(mb_tags, track.s.id) |> dplyr::pull(n)
votes_per_track <- mb_tags |> 
...
There were `r format(n_unique_mb_tags, big.mark = ',')` different genre tags in the MusicBrainz genre data set. Users assigned between `r min(tags_per_track)` and `r max(tags_per_track)` different tags to each track (median: `r median(tags_per_track)`, IQR: `r IQR(tags_per_track)`) and voted for each track between `r min(votes_per_track)` and `r max(votes_per_track)` times (median: `r median(votes_per_track)`, IQR: `r IQR(votes_per_track)`).
