...
\setcounter{figure}{0} 
\section{RQ1 Detailed COMGET Algorithm For Constructing Balanced Hierarchical Trees}
\vspace{1.5cm}
\begin{tikzpicture}[
    ...
    \node[terminator, fill = red!60] (A) {\textbf{Start from} popular music corpus with multilabel \textit{genre} tags assigned to all tracks};
    \node[process, right=of A, text width = 10cm] (B) {Build a weighted co-ocurrence network of \textit{genres} with directed edges \textit{e}. For each ordered pair of \textit{genres} $(A,B)$ compute vote-weighted conditional frequency:\\\begin{equation*}\label{eq:initial_network}
        e_{A\rightarrow B} = P(B \mid A)\ \times\ \frac{1}{|T_{A,B}|}\sum_{t\in T_{A,B}}
        \frac{v_{t,B}}{v_{t,\text{total}}}
    \end{equation*} with $T_{A,B}$ the tracks tagged with both $A$ and $B$, $v_{t,B}$ the votes for $B$ on track $t$, and $v_{t,\text{total}}$ the total votes on $t$};
    \node[process, below=of B, text width = 10cm] (C) {Resolve reciprocal links and keep only the positive residual of the stronger direction:\begin{align*}\label{eq:remove_weaker_edge}
    e'_{A\to B} &= \big(e_{A\to B}-e_{B\to A}\big)_+ \\
    e'_{B\to A} &= \big(e_{B\to A}-e_{A\to B}\big)_+
    \end{align*}};
    \node[process, below=of C, text width = 10cm] (D) {Enforce a strict tree by assigning each node its single strongest outgoing edge as its \textit{supergenre}, discarding all other edges. Attach components lacking a \textit{supergenre} to an artificial root descriptive of the corpus};
    \node[process, below left=6mm and -47.5 mm of D, fill=blue!40] (E) {For each edge $e_i$ in tree (from weakest to strongest):};
    \node[process, right=7.5 mm of E] (F) {Remove $e_i$ from tree};
    \node[process, below=12mm of F] (G) {Attach dangling subtree to artificial root};
    \node[process, below=11mm of G] (H) {Calculate \textit{imbalance score} as weighted average of Gini within hierarchy levels};
    \node[process, left=7.5 mm of H] (I) {Store \textit{imbalance score} and current tree state for later reference};
    \node[decision, below=of E] (J) {Are all edges connected to the artificial root?};
    \node[process, left=of J] (K) {Choose tree state with lowest \textit{imbalance score}};
    \node[terminator, fill=green!60, below=12mm of K] (L) {COMGET};
 
    \draw[->] (A) -- (B);
    \draw[->] (B) -- (C);
    \draw[->] (C) -- (D);
    \draw[->] (D.south) --+ (0, -0.2) -| (E);
    \draw[->] (E) -- (F);
    \draw[->] (F) -- (G);
    \draw[->] (G) -- (H);
    \draw[->] (H) -- (I);
    \draw[->] (I) -- (J);
    \draw[->] (J) -- node[anchor=north] {Yes} (K);
    \draw[->] (J) -- node[anchor=east] {No} (E);
    \draw[->] (K) -- (L);
    
\end{tikzpicture}

\newpage
\section{RQ2 Detailed FOLDCAT Flowchart for Folding COMGET to Identify Genre Categories}
\vspace{0.5cm}
...
\begin{tikzpicture}[
    ...
    \node[process, fill=green!30, right=of B] (C) {\textbf{Final hyperparameter tuning} on full data set with chosen learner};
    \node[terminator, fill=yellow!30, below=of C] (D) {\textbf{Final model training and evaluation} on full data set with chosen learner};
...
\begin{tikzpicture}[
    ...
    \node[process, below=10.5mm of A] (D) {dummy code categorical features, reference level: most common level};
    \node[process, below=8.5mm of B] (E) {seeded split into train and holdout set (80/20) on artist level to mitigate artist effects~\citep{flexerEffectsAlbumArtist2010}};
    \node[process, below=of C] (F) {draw seeded and outcome-stratified 20\% subsample from training data and create 5 seeded CV folds from subsampled training data on artist level~\citep{flexerEffectsAlbumArtist2010}};
    \node[process, below=13mm of D] (G) {remove dummy-coded categorical features with less than 100 observations in any level in any fold on the subsample in full training and holdout set};
    \node[terminator, below=12.5mm of E] (H) {Training-ready train and holdout data set};
...
\begin{tikzpicture}[
    ...
    \node[process, right=of B] (C) {create 5 seeded CV folds from subsampled training data on artist level~\citep{flexerEffectsAlbumArtist2010} for lower levelsl of granunarlity (5, 12)};
    \node[process, below=10mm of A] (D) {perform hyperparameter search individually for GLMNET, RDA, RF, and lightGBM learner: Bayesian optimization~\citep{snoekPracticalBayesianOptimization2012a} with 10x inital latin hypercube grid~\citep{husslageSpacefillingLatinHypercube2011} and max. 25 iterations, 10x no improvement stop criterion, 5x no improvement jump to max. uncertainty};
    \node[process, below=19mm of B] (E) {select best learner hyperparameters based on $\text{F1}_{\text{macro}}$ and one standard error rule};
    \node[process, below=16.5mm of C] (F) {evaluate and compare performance across CV folds and select best learner};
    \node[terminator, below=of F] (G) {Best learner for final modeling};

    \draw[->] (A) -- (B);
    \draw[->] (B) -- (C);
    \draw[->] (C.south) --+ (0, -0.25) -| (D);
    \draw[->] (D) -- (E);
    \draw[->] (E) -- (F);
    \draw[->] (F) -- (G);
...
\vspace{1 cm}
\subsection{Final Hyperparameter Tuning}
\begin{tikzpicture}[
    node distance=6mm and 5mm,
    ...
    \node[process, right=of B] (C) {perform hyperparameter search for selected learner: Bayesian optimization~\citep{snoekPracticalBayesianOptimization2012a} with 20x initial latin hypercube grid~\citep{husslageSpacefillingLatinHypercube2011} and max. 50 iterations, 10x no improvement stop criterion, 5x no improvement jump to max. uncertainty};
    \node[process, below=16mm of A] (D) {select best learner hyperparameters based on $\text{F1}_{\text{macro}}$ and one standard error rule~\citep{hastieModelAssessmentSelection2009}};
    \node[terminator, below=16mm of B] (E) {hyperparameters used for final model training};
...
\vspace{1 cm}
\subsection{Final Model Training and Evaluation}
\begin{tikzpicture}[
    node distance=6mm and 5mm,
    terminator/.style = {rectangle, draw, align=flush left, text width = 4.5cm, rounded corners, minimum height=2em, font = \footnotesize, fill=yellow!30},
    process/.style = {rectangle, draw, align=flush left, text width = 4.5cm, minimum height=2em, font = \footnotesize, fill=yellow!30},
    >=Stealth]
\node[terminator] (A) {full training data and selected hyperparemters};
\node[process, right=of A] (B) {train final models on full training data with chosen hyperparameters};
\node[process, right=of B] (C) {evaluate final models on holdout set};
\node[terminator, below=of C] (D) {model evaluation };
    \draw[->] (A) -- (B);
    \draw[->] (B) -- (C);
    \draw[->] (C) -- (D);
\end{tikzpicture}
...
\begin{tikzpicture}[
    ...
    \node[process, below=14mm of A] (D) {NearMiss~\citep{zhangKNNApproachUnbalanced2003} downsampling according to tuned target ratio};
    \node[process, below=12mm of B] (E) {Borderline SMOTE upsample~\citep{hanBorderlineSMOTENewOverSampling2005} (non-sufficient danger observations: regular SMOTE upsample) according to tuned target ratio};
...
\end{tabular}
\caption{Hyperparameter settings for final LightGBM model training at different granularity levels.}
...
\begin{figure}[H]
  ...
  \begin{subfigure}[b]{\textwidth}
    ...
    \caption{COMGET-G32}
  \end{subfigure}
  \caption{Confusion matrices for the final LightGBM genre classification models for COMGET-G25 and COMGET-G32 on the holdout set. Values show $P(\text{Predicted}|\text{Actual})$; \textit{certainty} indicates the mean assignment probability for each \textit{genre category} across community votes.}
...
\begin{figure}[H]
  ...
  \begin{subfigure}[b]{\columnwidth}
    ...
    \caption{COMGET-G32}
  \end{subfigure}
  \caption{Top 20 most important features for algorithmic genre classification of POPTRAG corpus at high (25) and very high (32) granularity of COMGET-G. Values show relative feature importance extracted from final LightGBM models.}
...
\noindent
\textit{Link is left out for the purpose of anonymization of authors. Please find an interactive COMGET explorer .html file attached to this submission.}
